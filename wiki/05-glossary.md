...
| **tray_action** | Pure `fn(&str) -> Option<TrayAction>` in `src-tauri/src/tray.rs`. Maps tray menu ids (`show`/`hide`/`quit` via `MENU_*` consts) to `TrayAction::{Show,Hide,Quit}`; unknown → `None`. The unit-tested seam of the otherwise GUI-bound tray (Phase 16) |
| **hide-to-tray** | Window X (`CloseRequested`) is intercepted in `lib.rs .on_window_event` → `prevent_close()` + `hide()` + `emit("close-to-tray")`. App keeps running; tray **Quit** (`app.exit(0)`) is the only full exit (Phase 16) |
| **close-to-tray event** | Tauri event emitted by the backend on each hide-to-tray. Frontend `on_close_to_tray_once` (wraps `event.once`) shows an info toast at most once per app run |
| **tray-icon feature** | Cargo feature on the `tauri` dep enabling first-party tray support. No extra crate added (Phase 16) |
