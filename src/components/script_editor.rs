...
pub fn ScriptEditor() -> impl IntoView {
    ...
    let script_data = create_resource(editing_id, move |id| async move {
        match id {
            None => Ok(None),
            Some(id) => tauri_api::get_script(&id).await.map(Some),
        }
    });
    ...
    let schedule_auto_save = move || {
        ...
        let cb = Closure::wrap(Box::new(move || {
            ...
            spawn_local(async move {
                let result = match id {
                    None => tauri_api::create_script(&t, &c).await,
                    Some(ref id) => tauri_api::update_script(id, &t, &c).await,
                };
                match result {
                    Ok(script) => {
    ...
    let on_start_prompter = move |_: leptos::ev::MouseEvent| {
        let t = title.get();
        let c = content.get();
        if !t.trim().is_empty() {
            if is_existing.get() {
                ...
                set_save_status.set(SaveStatus::Saving);
                spawn_local(async move {
                    if let Ok(script) = tauri_api::create_script(&t, &c).await {
                        app_state.selected_script_id.set(Some(script.id.clone()));
                        app_state.editing_script_id.set(Some(script.id));
                        is_existing.set(true);
                    }
